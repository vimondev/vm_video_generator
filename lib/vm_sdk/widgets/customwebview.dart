...
class CustomWebView extends StatelessWidget {
  final Function(InAppWebViewController) callback;
  final Function() handleTerminated;
  final String? initialFile;

  const CustomWebView({Key? key, required this.callback, required this.handleTerminated, this.initialFile})
      : super(key: key);
  ...
  Widget build(BuildContext context) {
    ...
    return InAppWebView(
        ...
        onWebViewCreated: callback,
        iosOnWebContentProcessDidTerminate: (_) {
          handleTerminated();
        },
        onConsoleMessage: (controller, consoleMessage) {
          if (kDebugMode) {
            print('CustomWebView - $consoleMessage');
          }
        });
  }
}
