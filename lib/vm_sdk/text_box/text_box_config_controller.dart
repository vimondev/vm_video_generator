...
class TextBoxConfigController {
  ...
  Future<String> renderImageAndSave() async {
    ...
    var outerSize = Size(newSize.size.width, newSize.size.height + ((padding.top + padding.bottom) / (scaleSetter?.call() ?? 1.0)));
    updateSize(outerSize);

    ui.Image renderedImage = await recorder.endRecording().toImage(imageSize.width.floor(), imageSize.height.floor());
